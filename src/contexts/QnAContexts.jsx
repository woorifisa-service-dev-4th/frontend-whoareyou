...
const reducer = (state, action) => {
  ...
  switch (action.type) {
    ...
    case "COMMENT_UPDATE":
      const updateComments = state.commentData.map((comment) =>
        comment.id === action.updatedComment.id
          ? action.updatedComment
          : comment
      );
      return { ...state, commentData: updateComments };
    case "RECOMMENT_UPDATE":
      const updateReComments = state.commentData.map((comment) =>
        comment.id === action.updatedComment.id
          ? action.updatedComment
          : comment
      );
    ...
    case "COMMENT_DELETE":
      const deleteComments = state.commentData.map(
        (comment) => comment.id !== action.deletedComment.id
      );
      return { ...state, commentData: deleteComments };
    case "RECOMMENT_DELETE":
      const deleteReComments = state.commentData.map(
        (comment) => comment.id !== action.deletedComment.id
      );
